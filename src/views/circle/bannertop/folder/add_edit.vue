 ...
 * @LastEditTime: 2019-11-21 15:50:15
 -->
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="180px" class="demo-form-inline">
      ...
      <el-form-item :label="form.actionType == '1' ? '网页链接地址' : 'DAppId'" prop="actionData">
        ...
        <el-popover v-if="form.actionType == '2'" placement="right-start" width="300" height="200">
          ...
          <el-button slot="reference" type="primary" plain>点击查看DApp列表</el-button>
        </el-popover>
      </el-form-item>
...
import { addBannerTop, editBannerTop } from '@/api/circleModule/bannertop'
export default {
  data() {
    return {
      ...
      rules: {
        ...
        actionData: [
          { required: true, message: '请填写操作类型数据', trigger: 'change' }
        ],
        title: [
    ...
    submitHandler() {
      ...
      if (this.title.state === 'edit') {
        editBannerTop(params).then(v => {
          ...
          this.$router.push({
            path: '/circle/bannertop/bannertop-list'
          })
        })
      } else if (this.title.state == 'add') {
        addBannerTop(params).then(v => {
          this.$notify.success({
            title: '成功',
            message: `${this.title.text}成功`
          })
