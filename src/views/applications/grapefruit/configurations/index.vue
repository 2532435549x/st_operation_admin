 ...
 * @LastEditors: 熊成强
 * @LastEditTime: 2019-09-03 09:07:56
 -->
<template>
  <div class="contarin">
    <div style="padding:10px 10px">
      <el-input
        v-model="params1.s_LIKE_cfgName"
      ...
      <el-input
        v-model="params1.s_LIKE_cfgGroup"
        placeholder="请输入配置类型"
        style="width: 200px;"
        class="filter-item"
        clearable
      ...
      />
      <el-input
        v-model="params1.s_LIKE_cfgValue"
      ...
      />
      <el-input
        v-model="params1.s_LIKE_description"
        type="text"
        placeholder="请输入配置说明"
        style="width: 200px;"
        class="filter-item"
    ...
    >
      ...
      <el-table-column label="创建时间" width="235" align="center">
        <template slot-scope="scope">
          <div>{{ scope.row.createTime }}</div>
        </template>
      </el-table-column>

      <el-table-column label="修改时间" width="235" align="center">
        <template slot-scope="scope">
          <div>{{ scope.row.updateTime || '暂无数据' }} </div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getDlist() {
      const params = Object.assign({}, this.params, this.params1)
