 ...
 * @LastEditors: wangyun
 * @LastEditTime: 2019-11-11 10:57:01
 -->
<template>
  <div class="app-container">
    <div class="page-header">
      <el-page-header @back="$router.go(-1)" content="账户注册列表"></el-page-header>
    </div>
    <div class="app-container-flex">
      <div class="filter-container container-flex container-flex1">
        <el-form>
          ...
          <el-form-item label="注册用户名">{{ detailData.account }}</el-form-item>
        ...
        <el-collapse :accordion="accordion">
          <el-collapse-item title="订单附加数据" v-if="detailData.orderData">
            <el-form>
              ...
              <el-form-item label="注册用户名">{{ detailData.orderData.account }}</el-form-item>
              <el-form-item label="注册账户公钥">{{ detailData.orderData.publicKey }}</el-form-item>
            </el-form>
          </el-collapse-item>
...
<script>
import { regAccountOrderDetail } from '@/api/resourceRent/order'
...
export default {
  ...
  created() {
    if (this.$route.query.id) {
      regAccountOrderDetail({ id: this.$route.query.id }).then(v => {
