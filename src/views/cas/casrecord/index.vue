<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="加减积分数量" sortable="score" align="center" width="140">
        <template slot-scope="scope">{{ scope.row.score }}</template>
      </el-table-column>

      <el-table-column label="积分加减类型" align="center" width="150">
        <template slot-scope="{row}">
          <el-tag v-if="row.type == 1" type="success">添加</el-tag>
          <el-tag v-if="row.type == 2" type="danger">扣减</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="英文备注" align="center" width="150">
        <template slot-scope="scope">{{ scope.row.memoEn }}</template>
      </el-table-column>

      <el-table-column label="注册时间" sortable="create_time" align="center" width="200">
...
<script>
import { getCasRecordList } from '@/api/casModule'
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        let res = await getCasRecordList(this.queryList)
