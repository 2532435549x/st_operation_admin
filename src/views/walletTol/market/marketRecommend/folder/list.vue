<!--
 * @Description: marketRecommend -> list
 * @Author: wangyun
 * @Date: 2019-09-16 11:41:51
 * @LastEditors: wangyun
 * @LastEditTime: 2019-11-28 16:54:36
 -->
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-form :inline="true" class="demo-form-inline">
        <el-form-item>
          <el-input v-model="queryList.s_LIKE_fromSymbol" placeholder="请输入币种" clearable />
        </el-form-item>

        <el-form-item>
          <el-input v-model="queryList.s_LIKE_fromContract" placeholder="请输入合约名称" clearable />
        </el-form-item>

        <!-- <el-form-item>
          <el-select v-model="queryList.s_LIKE_chainType" clearable placeholder="选择兑换链类型">
    ...
    >
      ...
      <el-table-column label="币种" align="center">
        <template slot-scope="scope">{{ scope.row.fromSymbol }}</template>
      </el-table-column>

      <el-table-column label="合约名称" align="center">
        <template slot-scope="scope">{{ scope.row.fromContract }}</template>
      </el-table-column>

      <el-table-column label="兑换币种" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.toSymbol=='EOS'">{{ scope.row.toSymbol }}</el-tag>
          <el-tag v-else-if="scope.row.toSymbol=='BOS'" type="success">{{ scope.row.toSymbol }}</el-tag>
          <el-tag v-else type="warning">{{ scope.row.toSymbol }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column label="兑换链类型" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.toChainType=='EOS'">{{ scope.row.toChainType }}</el-tag>
          <el-tag
            v-else-if="scope.row.toChainType=='BOS'"
            type="success"
          >{{ scope.row.toChainType }}</el-tag>
          <el-tag v-else type="warning">{{ scope.row.toChainType }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="title.text">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" class="demo-ruleForm">
        <el-form-item label="币种" prop="fromSymbol">
          <el-input v-model="form.fromSymbol" placeholder="请填入币种" />
        </el-form-item>

        <el-form-item label="合约名称" prop="fromContract">
          <el-input v-model="form.fromContract" placeholder="请填入合约" />
        </el-form-item>

        <el-form-item label="兑换币种" prop="toSymbol">
          <el-input v-model="form.toSymbol" placeholder="请填入兑换的币种" />
        </el-form-item>

        <el-form-item label="兑换链类型" prop="toChainType">
          <el-select v-model="form.toChainType" placeholder="请选择兑换链">
            <el-option
              v-for="(item, index) in chainTypeOptions"
              :key="index"
              :label="item.label"
...
import {
  getMarketRecommendList,
  getMarketRecommendDetail,
  addMarketRecommend,
  editMarketRecommend,
  deleteMarketRecommend
...
export default {
  data() {
    return {
      ...
      rules: {
        fromSymbol: [
          { required: true, message: '请填入来源币种', trigger: 'blur' }
        ],
        fromContract: [
          { required: true, message: '请填入来源合约', trigger: 'blur' }
        ],
        toSymbol: [
          { required: true, message: '请填入兑换币种', trigger: 'blur' }
        ],
        toChainType: [
          { required: true, message: '请填入兑换链', trigger: 'blur' }
        ],
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        let res = await getMarketRecommendList(this.queryList)
        console.log('res: ', res)
    ...
    stateExchange(item, state) {
      this.$confirm(
        ...
        .then(_ => {
          ...
          editMarketRecommend(params).then(v => {
            this.$notify.success({
              title: '成功',
              message: '修改成功'
            })
    ...
    submitHandler() {
      ...
      if (this.title.state === 'edit') {
        ...
        addMarketRecommend(params).then(v => {
          if (v.code === 1000) {
            this.$notify.success({
              title: '成功',
              message: `${this.title.text}成功`
    ...
    delItem(item) {
      this.$confirm(
        ...
        .then(() => {
          deleteMarketRecommend({ ...item }).then(v => {
            if (v.code == 1000) {
              this.$notify.success({
                title: '成功',
                message: '删除成功'
